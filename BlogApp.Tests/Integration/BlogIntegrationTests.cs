...
public class BlogIntegrationTests : IClassFixture<SqlServerContainerFixture>, IAsyncLifetime
{
    ...
    //[Fact]
    //public async Task DeleteComment_WithValidAdminKeyAndToken_ShouldReturnViewWithoutDeletedItem()
    //{
    //    // Arrange
    //    var posts = TestHelper.CreatePosts(1);
    //    using var scope = _app.Services.CreateScope();
    ...
    //    await context.SaveChangesAsync();
    //    _client.DefaultRequestHeaders.Add("admin_key", "test-admin-key");
    //    var route = $"/posts/{posts.First().Slug}/delete-comment?token={comments[^1].Token}";
    ...
    //    // Assert
    //    Assert.DoesNotContain(comments[^1].Token, resultString);
    //}

    [Fact]
    public async Task DeleteComment_WithInvalidAdminKey_ShouldReturnUnauthorized()
    {
        // Arrange
        var posts = TestHelper.CreatePosts(1);
        using var scope = _app.Services.CreateScope();
    ...
    [Fact]
    public async Task DeleteComment_WithInvalidToken_ShouldReturnBadRequest()
    {
        // Arrange
        var posts = TestHelper.CreatePosts(1);
        using var scope = _app.Services.CreateScope();
        ...
        await context.SaveChangesAsync();
        _client.DefaultRequestHeaders.Add("admin_key", "test-admin-key");
        var route = $"/posts/{posts.First().Slug}/delete-comment?token=invalid-token";
